...
class Robot:
    ...
    def battery(self):
        resp = requests.get('http://'+self.ip+'/api/battery').json()
        battery_percentage = resp['result']['chargePercent'] * 100
    ...
    def populateImages(self):
        self.images_saved = []
        resp = requests.get('http://'+self.ip+'/api/images/list').json()
        self.images_saved = [item['name'] for item in resp['result']]       

    def populateAudio(self):
        self.audio_saved = []
        resp = requests.get('http://'+self.ip+'/api/audio/list').json()
        self.audio_saved = [item['name'] for item in resp['result']]   
